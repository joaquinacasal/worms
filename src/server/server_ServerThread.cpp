...
void ServerThread::send_stage_information_to_clients(int width, int height){
  if (!keep_running) return;
  for (size_t i = 0; i < clients.size(); ++i){
    notifier.send_stage_info(clients[i], width, height);
  }
}
...
void ServerThread::send_radiocontrolled_information_to_clients(int x, int y){
  if (!keep_running) return;
  for (size_t i = 0; i < clients.size(); ++i){
    notifier.send_radiocontrolled_info(clients[i], x, y);
  }
}
