    ...
    ui(new Ui::MainWindow)
{
    ...
    setWindowIcon(QIcon(QString(LABEL_IMAGES_DIR) + \
                                QString(WORM_IMAGE_FILENAME)));
...
void MainWindow::set_widget_size(int width, int height)
{
    ...
    scenario_widget->resize(scenario.meters_to_pixels(width),\
                            scenario.meters_to_pixels(height));
}

void MainWindow::load_background(QString filename){
    QPixmap background(BACKGROUND_IMAGES_DIR + filename);
    background = background.scaled(scenario_widget->size(),\
...
void MainWindow::on_actionLoad_background_triggered()
{
    QString filename = QFileDialog::getOpenFileName(this,\
                              SELECT_BACKGROUND_MESSAGE, BACKGROUND_IMAGES_DIR);
    if (filename == "")
        return;
...
{
    QString filename = QFileDialog::getOpenFileName(this, \
                                  SELECT_SCENARIO_MESSAGE, SCENARIO_FILES_DIR);
    if (filename == "")
        return;
    ...
    for (map<string, string> worm : worms){
        ...
        DragLabel* new_worm = new DragLabel(image_filename, scenario_widget,\
                                        false, WORM_SIZE, WORM_SIZE, 0, true);
        new_worm->move(x, y);
        new_worm->show();
    ...
    for (map<string, string> beam : beams){
        ...
        DragLabel* new_beam = new DragLabel(image_filename, scenario_widget, \
                                          false, length, height, angle, false);
        new_beam->move(x, y);
        new_beam->show();
...
void MainWindow::on_actionSave_scenario_triggered()
{
    ...
    if (current_file.isEmpty()) {
        filename = QFileDialog::getSaveFileName(this, SAVE_SCENARIO_MESSAGE,\
                                                SCENARIO_FILES_DIR);
        if (filename == ""){
            return;
        }
    ...
    int status = scenario.save_scenario(worms_labels, beams_labels, \
...
void MainWindow::on_actionSave_scenario_as_triggered()
{
    QString filename = QFileDialog::getSaveFileName(this, \
                                    SAVE_SCENARIO_MESSAGE, SCENARIO_FILES_DIR);
    if (filename == ""){
        return;
    }
    current_file = filename;
...
{
    QDialog* input_dialog = new Dialog(this, scenario.get_width(), \
                                       scenario.get_height());
    input_dialog->setAttribute(Qt::WA_DeleteOnClose);
    input_dialog->show();
}
