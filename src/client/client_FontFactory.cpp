...
FontFactory::FontFactory(){
    if (TTF_Init() < 0)
        throw SdlException("No se pudo inicializar la librería TTF", \
                            TTF_GetError());
...
SDL_Texture* FontFactory::get_texture_big_font(const char* text,\
                                               SDL_Color color, \
...
SDL_Texture* FontFactory::get_texture_small_font(const char* text, \
                                                 SDL_Color color, \
                                                 SDL_Renderer* renderer){
