...
void TextureFactory::init(SDL_Renderer* renderer){
    textures["worm_resting"] = load_texture(string(ASSETS_FOLDER) + \
                                        string(WORM_RESTING_ASSET), renderer);
    textures["worm_walking"] = load_texture(string(ASSETS_FOLDER) + \
                                        string(WORM_WALKING_ASSET), renderer);
    textures["worm_jumping"] = load_texture(string(ASSETS_FOLDER) + \
                                        string(WORM_JUMPING_ASSET), renderer);
    textures["beam"] = load_texture(string(ASSETS_FOLDER) +\
                                        string(BEAM_ASSET), renderer);
    textures["start_turn"] = load_texture(string(ASSETS_FOLDER) + \
                                        string(START_TURN_ASSET), renderer);
...
SDL_Texture* TextureFactory::load_texture(const string& filename,\
