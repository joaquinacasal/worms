...
MunitionsInformation::MunitionsInformation(double x, double y,\
  ...
  titulo = font_factory.get_texture_small_font(title_message.c_str(), \
                        colors_factory.get_color_by_name("black"), renderer);
...
int MunitionsInformation::render() {
  ...
  SDL_Rect munitions_info_rect_teletransportacion = \
                                            rect_teletransportacion.toRect();
  ...
  SDL_RenderCopy(this->renderer, teledirigido, NULL,\
                 &munitions_info_rect_teledirigido);
  SDL_RenderCopy(this->renderer, teletransportacion, NULL,\
                 &munitions_info_rect_teletransportacion);
}
...
void MunitionsInformation::set_dynamite_munitions(int dynamite_q) {
  ...
  dinamita = font_factory.get_texture_small_font(dynamite_message.c_str(), \
                          colors_factory.get_color_by_name("black"), renderer);
...
void MunitionsInformation::set_radiocontrolled_munitions(int radiocontrolled_q){
  ...
  teledirigido = font_factory.get_texture_small_font(\
                  radiocontrolled_message.c_str(), \
                  colors_factory.get_color_by_name("black"), renderer);
  last_radiocontrolled_munitions = radiocontrolled_q;
}

void MunitionsInformation::set_teletransportation_munitions(\
  ...
  std::string teletransportation_message = "Teletransportacion: " +\
                                           teletransportation_m;
  if (teletransportacion)
    SDL_DestroyTexture(teletransportacion);
  teletransportacion = font_factory.get_texture_small_font(\
                                    teletransportation_message.c_str(),\
                                    colors_factory.get_color_by_name("black"),\
                                    renderer);
  last_teletransportation_munitions = teletransportation_q;
}

void MunitionsInformation::set_munitions(int dynamite_q,\
                                         int radiocontrolled_q, \
                                         int teletransportation_q) {
